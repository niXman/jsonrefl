...
#include "jsonrefl_validation_paths.hpp"
...
#ifndef _WIN32
static std::string env_path_native(const char *s)
...
static std::string env_path_native(const char *s)
{
    ...
    if ( s[0] == '/' && std::isalpha(static_cast<unsigned char>(s[1])) && s[2] == '/' ) {
        std::string t;
        t += static_cast<char>(std::toupper(static_cast<unsigned char>(s[1])));
        t += ':';
...
{
    const char *e = std::getenv("JSONREFL_VALIDATION_SOURCE_DIR");
    if ( e != nullptr && e[0] != '\0' )
        return env_path_native(e);

    const char *gw = std::getenv("GITHUB_WORKSPACE");
    if ( gw != nullptr && gw[0] != '\0' )
        return path_join(env_path_native(gw), "validation");

    return jsonrefl_validation_detail::source_dir();
}
...
static bool parse_json(const char *filename, T &obj) {
    auto json = read_file(path_join(path_join(fixture_root(), "roundtrip"), filename));
    // accum is required for escape-bearing strings; supply one unconditionally
    // so the parser can decode escapes into it.
    std::string accum;
    auto p = jsonrefl::make_parser(&obj, &accum);
    return p.parse(json.data(), json.size()).status() == jsonrefl::state::ok;
}
...
static jc_result parse_jc(const char *filename, T &obj) {
    auto json = read_file(path_join(path_join(fixture_root(), "jsonchecker"), filename));
    std::string accum;
    auto p = jsonrefl::make_parser(&obj, &accum);
    const auto st = p.parse(json.data(), json.size()).status();
    const bool ok = (st == jsonrefl::state::ok || st == jsonrefl::state::incomplete);
    const bool finished = (st == jsonrefl::state::ok);
    return {ok, finished};
}
...
struct roundtrip10 {
    jsonrefl::optional_t<std::string> a;
    std::string foo;
};
...
// REJECT: invalid \x escape sequence
void test_fail15() {
    std::vector<std::string> obj;
...
// REJECT: invalid \0 escape sequence
void test_fail17() {
    std::vector<std::string> obj;
...
// REJECT: invalid backslash-space escape sequence
void test_fail26() {
    std::vector<std::string> obj;
...
// REJECT: invalid backslash-newline escape sequence
void test_fail28() {
    std::vector<std::string> obj;
